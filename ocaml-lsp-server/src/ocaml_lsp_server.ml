  ...
  let capabilities =
    ...
    let executeCommandProvider =
      ExecuteCommandOptions.create
        ~commands:
          (view_metrics_command_name :: Open_related.command_name
         :: Dune.commands)
        ()
    in
...
let on_request :
 ...
 fun server req ->
  ...
  | ExecuteCommand command ->
    ...
    else if String.equal command.command Open_related.command_name then
      later
        (fun _state server -> Open_related.command_run server command)
        server
    else
      later
        (fun state () ->
